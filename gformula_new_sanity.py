    ...
    if not os.path.exists(f'./{data_dir}/'):
        ...
        for i in tqdm.trange(seeds):
            ...
            if test_stat == 3:
                ...
                c = weighted_stat(X=X,Y=Y,Z=Z,w=w,cuda=True,device=device)
            elif test_stat == 1:
        ...
        plt.savefig(f'./{data_dir}/p_value_plot_null=True_test_stat={test_stat}.png')
        plt.clf()
        p_value_array = torch.tensor(p_value_list)
        torch.save((p_value_array+1e-3).log(),f'./{data_dir}/p_value_array.pt')
        plt.hist(p_value_array.numpy(),bins=seeds//10)
        plt.savefig(f'./{data_dir}/logp_value_plot_null=True_test_stat={test_stat}.png')
