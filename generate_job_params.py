...
def generate_job_binary(n_list,net_width,net_layers,runs=1,seed_max=1000,estimate=False,directory='job_dir/',job_type='kc',dirname='',null_case=False):
    ...
    for n in n_list:
        for alp in [0.0, 2 * 1e-2, 4 * 1e-2, 6 * 1e-2, 8 * 1e-2, 1e-1]:
            data_dir = f'do_null_univariate_alp={alp}_null={null_case}_d=3'
            val_rate = 0.2
            h_str =data_dir
            if estimate:
                models_to_run = zip(['NCE_Q'],[1])
            else:
                models_to_run = zip(['real_weights'],[1])
            for mode in ['Q']:
                for width in net_width:
...
def generate_job_mixed(n_list,net_width,net_layers,runs=1,seed_max=1000,estimate=False,directory='job_dir/',job_type='kc'):
    ...
    for null_case in [True,False]:
        if null_case:
            data_dir = f"do_null_mix_100/beta_xy=[0, 0.0]_d_X={d}_d_Y={d}_d_Z={d}_n=10000_yz=[0.5, 0.0]_beta_XZ=0.5_theta={theta}_phi=2.0"
        else:
            data_dir = f"do_null_mix_100/beta_xy=[0, 0.2]_d_X={d}_d_Y={d}_d_Z={d}_n=10000_yz=[0.5, 0.0]_beta_XZ=0.5_theta={theta}_phi=2.0"
        for n in n_list:
            val_rate = 0.2
            h_str =data_dir
            if estimate:
...
if __name__ == '__main__':
    ...
    # generate_job_binary(n_list=[1000,5000,10000],net_layers=[3],net_width=[32],runs=1,seed_max=100,estimate=True,directory='do_null_binary_job_h0_d=3',job_type='kc_rule_new',dirname='',null_case=True)
    # generate_job_binary(n_list=[1000,5000,10000],net_layers=[3],net_width=[32],runs=1,seed_max=100,estimate=True,directory='do_null_binary_job_h1_d=3',job_type='kc_rule_new',dirname='',null_case=False)
    generate_job_mixed(n_list=[1000,5000,10000],net_layers=[3],net_width=[32],runs=1,seed_max=100,estimate=True,directory='do_null_binary_job_mixed_all',job_type='kc_rule_new')
