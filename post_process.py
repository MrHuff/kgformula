...
def return_file_name(q_fac,q_dist,a,b,br,nce,est):

    _tmp = f'qf={q_fac}_qd={q_dist}_m={mode}_s={a}_{b}_e={est}_est={nce}_sp={est}_p=Y_br={br}_v=1'
                        ...
                        for beta_xz in [0.25, 0.5, 0.01, 0.1, 0.0]:
                            for nce in ['NCE_Q']:
                                for by in [1e-3,1e-2,1e-1,0.1,0.25,0.5]:
                                    row = [beta_xz,q,q_d,bootstrap_runs,nce,d_Z,by]
                                    h_int = int(not by==1)
                                    ba = 0.0
                                    if d_X == 1 and by == 0.0:
                                        by = 0
                                    ...
                                    f_name=return_file_name(q,q_d,0,100,bootstrap_runs,nce,est)
                                    for nr_of_gpus in [1,2,3,4,5,6,7,8]:
                                        try:
                                            ...
                                            for i in range(nr_of_gpus):
                                                ...
                                                suffices.append(return_file_name(q,q_d,a,b,bootstrap_runs,nce,est))
                                            if est:
                                                str_= suffices[0]
