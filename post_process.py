...
def return_file_name(a,b,perm):
    ...
    _tmp = f'm=new_s={a}_{b}_e=False_est=NCE_sp=False_p={perm}'
    ...
    for macro_mode in [False,True]:
        for perm,nr_of_gpus in zip(['X','Y'],[3,3]):
            for beta_xz in [0.01]:
                ...
                for q in [1]:
                    row = [perm,beta_xz,q]
                    data_path = f'univariate_{seed_max}_seeds/univariate_test/'
                    ...
                    if not macro_mode:
                        ...
                        df_pval = pd.read_csv(PATH+f'fdf_m=Q_s=0_100_e=False_est=NCE_sp=False_p={perm}.csv')
