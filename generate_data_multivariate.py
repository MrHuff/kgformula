...
if __name__ == '__main__':
    ...
    d_X = 3
    d_Y = 3
    beta_xz = [0.0]+[round(1./(d_Z**2),3)]*(d_Z) #Some choices of theta and phi render w_true to be invalid. Why? We want do derive cases where w_true makes the test valid.
    #Both theta and phi controlls odds ratio and distribution of X...
    b_y =  0.5
    for beta_xy in [[0,b_y],[0,0]]:
        data_dir = f'beta_xy={beta_xy}_d_X={d_X}_d_Y={d_Y}_d_Z={d_Z}_n={n}_yz={yz}_beta_XZ={round(1./(d_Z**2),3)}'
