...
if __name__ == '__main__':
    for d_Z in [3,50]:
        ...
        d_X = 3
        d_Y = 3
        b_z = 0. #In higher dimensions, the take seems to be that one must be careful with how much dependence is created.
        ...
        for n in [100,1000,10000]:
            for beta_xy in [[0,b_y],[0,0]]:
                ...
                for i in range(seeds):
                    X,Y,Z,w = simulate_xyz_multivariate(n,oversamp=5,d_Z=d_Z,beta_xz=beta_xz,beta_xy=beta_xy,seed = i,yz=yz,d_X=d_X,d_Y=d_Y,phi=2*2**0.5,theta=4)
                    if i==0:
