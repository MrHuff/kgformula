...
def generate_data_mixed(args):
    def decorator(data_dir,n,d_Z,beta_xz,beta_xy,i,yz,d_X,d_Y,phi,theta,fam_x,fam_z,fam_y,sanity,null):
        X, Y, Z, inv_w = simulate_xyz_mixed_multivariate(n, oversamp=100, d_Z=d_Z, beta_xz=beta_xz,
                                                   beta_xy=beta_xy, seed=i, yz=yz, d_X=d_X, d_Y=d_Y,
                                                   phi=phi, theta=theta, fam_x=fam_x, fam_z=fam_z, fam_y=fam_y)
        return X,Y,Z,inv_w
    return decorator(*args)
...
if __name__ == '__main__':
    ...
    sanity = False
    # for d_X,d_Y,d_Z, theta,phi in zip( [1,3,3,3],[1,3,3,  3],[1,3,15,50],[2.0,4.0,16.0,16.0],[2.0,2.0,2.0,2.0]): #50,3
    for d_X,d_Y,d_Z, theta,phi in zip( [3,3],[3,  3],[15,50],[8.0,16.0],[2.0,2.0]): #50,3
    # for d_X,d_Y,d_Z, theta,phi in zip( [3],[3],[15],[16.0],[2.0]): #50,3
